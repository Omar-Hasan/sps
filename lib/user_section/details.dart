...
class _detailsState extends State<details> {
  ...
  void initState() {
    super.initState();
    description = mainUser.myController.text;
    double h = double.parse(mainUser.hController.text);
    double b = double.parse(mainUser.bController.text);
    double tw = double.parse(mainUser.twController.text);
    double tf = double.parse(mainUser.tfController.text);
    double r = double.parse(mainUser.rController.text);

    section_name sectionName = new section_name();
    switch(mainUser.x_section){
      case 'I_section':
        listName = sectionName.I_section();
        break;
      case 'U_section':
        listName = sectionName.U_section();
        break;
      case 'L_section':
        listName = sectionName.L0_section();
        break;
      case 'T_section':
        listName = sectionName.T0_section();
        break;
      case 'circle_section':
        listName = sectionName.circle_section();
        break;
      case 'HolwRec_section':
        listName = sectionName.HolwRec_section();
        break;
    }
    ComputeDetails computeDetails =
        new ComputeDetails(mainUser.x_section,h,b,tw,tf,r,0,r,tf);
    ...
    Widget _popupBody() {
      return Container(
        ...
        child: PhotoView.customChild(
          ...
          child: SvgPicture.asset(mainUser.imageDetails),
        ),
      );
    }
  ...
  _refreshValue(String name) {
    description = name;
    ComputeDetails computeDetails =
        new ComputeDetails(mainUser.x_section, 0, 0, 0, 0, 0, 0, 0, 0);
    List<String> value = computeDetails.getvalue_detail(description);
    for (int i = 0; i < value.length; i++) {
